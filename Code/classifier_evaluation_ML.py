...
def evaluate(ground_truth, predictions, prediction_scores, decision_scores, classifier):
    '''
    ...
    if classifier == "SVM":
        loss = metrics.hinge_loss(ground_truth, decision_scores)
    ...
    if classifier == "SVM":
        return [accuracy, precision, recall, specificity, f1_score, MCC, AUROC, AUPR, loss]
    else :
