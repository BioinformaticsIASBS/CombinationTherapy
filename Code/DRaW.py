...
def train_dedti_model(path, ratio, result_path):
  ...
  # drug_mixed
  mixed = np.loadtxt('../Data/'+path+'/Similarity_Matrix_Drugs.txt')

  # protein_mixed 
  mixed_2 = np.loadtxt('../Data/'+path+'/Similarity_Matrix_Viruses.txt')

  # print('shape of drug mixed matrix: ',np.shape(mixed))
  # print('shape of protein mixed matrix: ', np.shape(mixed_2))

  with open('../Data/'+path+'/Y.npy', 'rb') as f:
      Y = np.load(f)

  with open('../Data/'+path+'/Xindex.npy', 'rb') as f:
      XIndex = np.load(f)
